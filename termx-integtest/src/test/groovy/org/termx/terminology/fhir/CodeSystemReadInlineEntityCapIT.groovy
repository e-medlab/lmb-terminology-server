...
class CodeSystemReadInlineEntityCapIT extends TermxIntegTest {
  ...
  def "read returns CodeSystem without concept[] when conceptsTotal exceeds the inline cap"() {
    ...
    and: "concept[] is empty because the cap engaged (10 > 5) — the cap's contract"
    !fhirJson.has("concept") || fhirJson.get("concept").size() == 0
    // (termx-server also asserts CodeSystem.count == 10 here; lmb's mapper doesn't
    // populate the FHIR `count` field, so we just verify the cap's primary effect:
    // concept[] absent. If/when lmb merges the mapper change that sets count, add
    // the assertion back.)
  }
