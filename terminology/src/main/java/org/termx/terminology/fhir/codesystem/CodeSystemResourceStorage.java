...
public class CodeSystemResourceStorage extends BaseFhirResourceHandler {
  ...
  private ResourceVersion load(String codeSystemId, String versionNumber) {
    ...
    // The inline-entity cap inside loadEntities (see field maxInlineEntities) is the
    // only protection on the read path against OOM-ing on large publishers. lmb
    // doesn't have upstream commit 7bb1410 yet (the ?_summary=true|text|count
    // short-circuit) so the call always tries to load — but the cap returns an
    // empty entity list above its ceiling and logs a WARN; clients still get an
    // accurate `count` from CodeSystemVersion.conceptsTotal.
